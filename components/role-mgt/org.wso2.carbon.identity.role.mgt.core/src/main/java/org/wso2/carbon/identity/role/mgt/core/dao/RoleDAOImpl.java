...
import static org.wso2.carbon.identity.role.mgt.core.dao.SQLQueries.DELETE_ROLE_WITH_AUDIENCE_SQL;
...
import static org.wso2.carbon.identity.role.mgt.core.dao.SQLQueries.UPDATE_HYBRID_ROLE_UUID_WITH_AUDIENCE_SQL;
...
public class RoleDAOImpl implements RoleDAO {
    ...
    protected void addHybridRoleID(String roleId, String roleName, String tenantDomain)
        ...
        try (Connection connection = IdentityDatabaseUtil.getUserDBConnection(true)) {
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                    UPDATE_HYBRID_ROLE_UUID_WITH_AUDIENCE_SQL)) {
                statement.setInt(RoleTableColumns.UM_TENANT_ID, tenantId);
                statement.setString(RoleTableColumns.UM_ROLE_NAME, roleName);
                statement.setString(RoleTableColumns.UM_UUID, roleId);
                statement.executeUpdate();
    ...
    public void deleteRole(String roleID, String tenantDomain) throws IdentityRoleManagementException {
        ...
        try (Connection connection = IdentityDatabaseUtil.getUserDBConnection(true)) {
            try {
                try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                        DELETE_ROLE_WITH_AUDIENCE_SQL, RoleTableColumns.UM_ID)) {
                    statement.setString(RoleTableColumns.UM_ROLE_NAME, roleName);
                    statement.setInt(RoleTableColumns.UM_TENANT_ID, tenantId);
                    statement.executeUpdate();
                }
